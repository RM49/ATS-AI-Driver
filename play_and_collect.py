...
STATE = {
    "running": True,
    "ai_enabled": True,  # Start with AI driving
    "keys_pressed": set()
}
...
def on_press(key):
    ...
    if hasattr(key, 'char') and key.char == 'p':
        ...
        # Important: Release all keys when switching modes to prevent sticky inputs
        release_all_keys()
        release_all_keys()
        return
...
def main():
    ...
    # Initialize frame stack for the model
    ...
    if not log_file_exists:
        writer.writerow(log_header) # Write header only if file is new
    ...
    # --- Main Loop ---
    steering_angle = 0.0 # For user intervention mode
    while STATE["running"]:
        ...
        # Ensure we have enough frames for a prediction
        if len(frame_stack) < FRAME_STACK_SIZE:
            continue
            
        # 2. Decide action based on current mode (AI vs. User)
        if STATE["ai_enabled"]:
            # --- AI DRIVING MODE ---
            # Prepare model input
            ...
            # Determine throttle from user input
            throttle = 1.0 if 'w' in STATE["keys_pressed"] else 0.0

            # Determine steering from user input (same logic as main.py)
